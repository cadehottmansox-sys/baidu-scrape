...
import auth
from searcher import search_platform, scan_single, build_brand_aware_query, build_freight_query, score_freight_forwarder
...
def brand_aware_search():
    ...
    try:
        results = asyncio.run(search_platform(
            query=enhanced, brand=brand, platform="all",
            mode="supplier", deep_scan=False, wechat_only=False, page_num=1))
        return jsonify({"ok":True,"original_query":query,"enhanced_query":enhanced,
...
def freight_search():
    ...
    destination = (data.get("destination") or "USA")
    cargo_type = (data.get("cargo_type") or "replica")
    query = build_freight_query(origin, destination, cargo_type)
    try:
        results = asyncio.run(search_platform(
            query=query, brand="", platform="baidu",
            mode="supplier", deep_scan=False, wechat_only=False, page_num=1))
        for r in results:
            r["ff_score"] = score_freight_forwarder(
