...
def submit_request(name, email, reason, ip, discord="", wechat="", password="", **kwargs):
    ...
    if existing and existing["status"] == "pending":
        ...
        existing["discord"] = discord; existing["wechat"] = wechat
        if password: existing["password_hash"] = _hash(password)
        _save(data)
        return {"ok": True, "status": "already_requested", "message": "Request updated!"}
    if not password or len(password) < 4:
        return {"ok": False, "error": "Password must be at least 6 characters."}
    req = {"id": secrets.token_hex(8), "name": name, "email": email,
           "reason": reason, "discord": discord, "wechat": wechat,
           "password_hash": _hash(password), "ip": ip,
...
def approve_request(req_id):
    ...
    if req["status"] == "approved": return {"status": "already_approved"}
    req["status"] = "approved"; req["approved_at"] = time.time()
    existing = next((u for u in data["approved"] if u["email"] == req["email"]), None)
    if existing:
        existing["approved"] = True; existing["revoked"] = False
        existing["password"] = req.get("password_hash")
    else:
        data["approved"].append({
            "name": req["name"], "email": req["email"],
            "password": req.get("password_hash"), "is_admin": False,
            "ip_history": [req["ip"]], "approved_at": time.time(),
            "last_login": None, "request_id": req_id,
            "search_count": 0, "last_search": None, "last_query": ""})
    _save(data)
...
def login_user(email, password, ip):
    ...
    if not user: return {"valid": False, "error": "Email not found or not approved yet."}
    if user.get("revoked"): return {"valid": False, "error": "Access revoked."}
    ...
    if not user.get("password"): return {"valid": False, "error": "Account not set up. Contact admin."}
...
def validate_token(token, ip):
    ...
    if expires_at and not user.get("is_admin") and email != OWNER_EMAIL:
        if time.time() > expires_at: return {"valid": False}
    is_admin = user.get("is_admin", False) or email == OWNER_EMAIL
    return {"valid": True, "name": user["name"], "email": email, "is_admin": is_admin}

def ensure_session(email, ip):
    data = _load()
    user = next((u for u in data["approved"] if u["email"] == email), None)
    if not user or user.get("revoked"): return None
    token = secrets.token_urlsafe(32)
    _SESSIONS[token] = email
    user["last_login"] = time.time()
    _save(data)
...
def set_expiry(email, expires_at):
    ...
    if expires_at is None: user.pop("expires_at", None)
    else: user["expires_at"] = expires_at
    _save(data)
...
def set_admin(email, is_admin=True):
    ...
    user = next((u for u in data["approved"] if u["email"] == email), None)
    if not user: return {"status": "not_found"}
    user["is_admin"] = is_admin; _save(data)
...
def update_password(email, new_password):
    ...
    user = next((u for u in data["approved"] if u["email"] == email), None)
    if not user: return {"status": "not_found"}
    user["password"] = _hash(new_password); _save(data)
