...
Stores users in a JSON file. Sends emails via Gmail SMTP.
"""
...
def _send_email(to: str, subject: str, html: str):
    ...
    if not GMAIL_USER or not GMAIL_PASS:
        print(f"[AUTH] No Gmail credentials — skipping email to {to}")
...
def _send_admin_notification(req: dict):
    approve_url = f"{APP_URL}/admin/approve/{req['id']}?secret={os.getenv('ADMIN_SECRET', 'changeme-set-in-env')}"
    deny_url    = f"{APP_URL}/admin/deny/{req['id']}?secret={os.getenv('ADMIN_SECRET', 'changeme-set-in-env')}"
    _send_email(
        to      = ADMIN_EMAIL,
...
<div style="font-family:Inter,sans-serif;max-width:500px;margin:0 auto;padding:24px">
  ...
  <table style="width:100%;border-collapse:collapse;font-size:14px">
    <tr><td style="padding:6px 0;color:#888;width:100px">Name</td><td style="color:#111;font-weight:600">{req['name']}</td></tr>
    <tr><td style="padding:6px 0;color:#888">Email</td><td style="color:#111">{req['email']}</td></tr>
    <tr><td style="padding:6px 0;color:#888">IP</td><td style="color:#111;font-family:monospace">{req['ip']}</td></tr>
    <tr><td style="padding:6px 0;color:#888;vertical-align:top">Reason</td><td style="color:#111">{req['reason'] or '—'}</td></tr>
  </table>
...
<div style="font-family:Inter,sans-serif;max-width:500px;margin:0 auto;padding:32px;background:#04101f;border-radius:12px">
  <h2 style="color:#7dd3fc;margin-bottom:8px">Hey {name}, you're in! 🎉</h2>
  <p style="color:#7aa5be;margin:12px 0 20px">Your access to SourceFinder has been approved.</p>
  <a href="{login_url}" style="display:inline-block;padding:12px 24px;background:#1a6fa4;color:#fff;text-decoration:none;border-radius:8px;font-weight:700">Access SourceFinder →</a>
