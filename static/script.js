...
function _sfTFF(el){var inner=document.getElementById("_sfTInner");if(inner){inner.style.width="min(720px,95vw)";inner.style.maxHeight="92vh";}el.innerHTML=_sH("FF Rate Calculator")+"<p style='color:#475569;font-size:12px;margin:0 0 16px'>Upload rate sheet photo. AI reads rates and estimates item weights. All in RMB yuan.</p>"+"<div onclick='document.getElementById(\'_ffFile\').click()' style='border:2px dashed rgba(34,211,238,.3);border-radius:12px;padding:16px;text-align:center;margin-bottom:14px;cursor:pointer;background:rgba(34,211,238,.04)'><div style='font-size:26px;margin-bottom:5px'>&#128247;</div><div style='color:#22d3ee;font-size:13px;font-weight:600'>Upload Rate Sheet Photo</div><div style='color:#334155;font-size:11px;margin-top:3px'>AI reads all rates automatically</div><input type='file' id='_ffFile' accept='image/*' style='display:none' onchange='_ffLoad(this)'></div>"+"<div id='_ffImgWrap' style='display:none;margin-bottom:14px;position:relative'><img id='_ffImgEl' style='width:100%;border-radius:10px;border:1px solid rgba(255,255,255,.1);max-height:200px;object-fit:contain;background:#000'><div id='_ffScanBadge' style='position:absolute;top:8px;right:8px;background:rgba(0,0,0,.85);border-radius:6px;padding:5px 10px;font-size:11px;color:#f59e0b;font-weight:600'>Scanning...</div></div>"+"<div id='_ffRatesBox' style='display:none;background:rgba(34,211,238,.05);border:1px solid rgba(34,211,238,.18);border-radius:10px;padding:12px;margin-bottom:14px'><div style='color:#22d3ee;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px'>Rates Detected (RMB/kg)</div><div id='_ffRatesList'></div></div>"+"<div style='color:#22d3ee;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-bottom:10px'>Your Items</div>"+"<div id='_ffItems'><div class='_ffItem' style='display:grid;grid-template-columns:1fr 65px 110px;gap:8px;margin-bottom:8px;align-items:center'><input class='_ffN' placeholder='Item (e.g. Jordan 4)' style='background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px 12px;border-radius:9px;font-size:13px;outline:none;box-sizing:border-box;font-family:inherit'><input class='_ffQ' type='number' placeholder='Qty' min='1' value='1' style='background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:9px;font-size:13px;outline:none;text-align:center;box-sizing:border-box;font-family:inherit'><select class='_ffS' style='background:#060c18;border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:9px;font-size:12px;outline:none;font-family:inherit'><option value=''>Shipping line...</option></select></div></div>"+"<div style='display:flex;gap:8px;margin-bottom:12px'><button onclick='_ffAddItem()' style='flex:1;padding:9px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#94a3b8;border-radius:9px;font-size:12px;cursor:pointer;font-family:inherit'>+ Add Item</button><button onclick='_ffCalcAI()' style='flex:2;padding:9px;background:linear-gradient(135deg,rgba(34,211,238,.2),rgba(99,102,241,.2));border:1px solid rgba(34,211,238,.35);color:#22d3ee;border-radius:9px;font-size:13px;cursor:pointer;font-weight:700;font-family:inherit'>Calculate with AI</button></div><div id='_ffResult'></div>";}
function _ffLoad(inp){var f=inp.files[0];if(!f)return;var rd=new FileReader();rd.onload=function(e){var img=document.getElementById("_ffImgEl"),wrap=document.getElementById("_ffImgWrap");if(img)img.src=e.target.result;if(wrap)wrap.style.display="block";window._ffB64=e.target.result.split(",")[1];window._ffMime=f.type||"image/jpeg";_ffScanChart();};rd.readAsDataURL(f);}
function _ffScanChart(){var badge=document.getElementById("_ffScanBadge"),rb=document.getElementById("_ffRatesBox"),rl=document.getElementById("_ffRatesList");if(badge){badge.textContent="AI scanning...";badge.style.color="#f59e0b";}if(rb)rb.style.display="none";fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:[{type:"image",source:{type:"base64",media_type:window._ffMime||"image/jpeg",data:window._ffB64}},{type:"text",text:"Freight forwarder rate chart. Extract shipping lines and per-kg rates in RMB. Return ONLY JSON array: [{line:string,rate:number,notes:string}]. Convert USD to RMB at 7.25."}]}]})}).then(function(r){return r.json();}).then(function(d){try{var text=(d.content||[]).map(function(b){return b.type==="text"?b.text:"";}).join("").replace(/```json|```/g,"").trim();var rates=JSON.parse(text);window._ffRates=rates;var opts=rates.map(function(r){return "<option value='"+r.rate+"'>"+r.line+" ("+r.rate+" RMB/kg)</option>";}).join("");document.querySelectorAll("._ffS").forEach(function(s){s.innerHTML="<option value=''>Select line...</option>"+opts;});if(rl)rl.innerHTML=rates.map(function(r){return "<div style='display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.05)'><span style='color:#94a3b8;font-size:12px'>"+r.line+"</span><span style='color:#22d3ee;font-size:12px;font-weight:700'>"+r.rate+" RMB/kg</span></div>"+(r.notes?"<div style='color:#334155;font-size:10px'>"+r.notes+"</div>":"");}).join("");if(rb)rb.style.display="block";if(badge){badge.textContent="Rates loaded";badge.style.color="#22c55e";}}catch(e){if(badge){badge.textContent="Could not read chart";badge.style.color="#ef4444";}}}).catch(function(){if(badge){badge.textContent="Scan failed";badge.style.color="#ef4444";}});}
...
function _ffCalcAI(){var res=document.getElementById("_ffResult");if(!res)return;var items=[];document.querySelectorAll("._ffItem").forEach(function(row){var name=(row.querySelector("._ffN")||{}).value||"",qty=parseInt((row.querySelector("._ffQ")||{}).value||1),rate=parseFloat((row.querySelector("._ffS")||{}).value||0);if(name)items.push({name:name,qty:qty,rate:rate});});if(!items.length){res.innerHTML="<span style='color:#ef4444'>Add at least one item</span>";return;}if(items.some(function(i){return !i.rate;})){res.innerHTML="<span style='color:#f59e0b'>Select a shipping line for every item</span>";return;}res.innerHTML="<div style='text-align:center;color:#f59e0b;padding:20px;font-size:13px'>AI estimating weights...</div>";var prompt="Estimate shipping weights for these streetwear items. Return ONLY JSON array no markdown: [{\"name\":string,\"qty\":number,\"kg_each\":number,\"rate\":number}]. Weights: sneakers 0.9kg, tshirt 0.25kg, hoodie 0.6kg, jacket 0.9kg, bag 0.8kg, hat 0.15kg, jeans 0.6kg, watch 0.15kg.\nItems:\n"+items.map(function(i){return i.qty+"x "+i.name+" (rate:"+i.rate+" RMB/kg)";}).join("\n");fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,messages:[{role:"user",content:prompt}]})}).then(function(r){return r.json();}).then(function(d){try{var text=(d.content||[]).map(function(b){return b.type==="text"?b.text:"";}).join("").replace(/```json|```/g,"").trim();var parsed=JSON.parse(text);var tRmb=0,tKg=0,tItems=0;var rows=parsed.map(function(it){var tkg=(it.kg_each||0)*it.qty,cost=tkg*(it.rate||0);tKg+=tkg;tRmb+=cost;tItems+=it.qty;return "<div style='display:flex;justify-content:space-between;align-items:center;padding:9px 12px;background:rgba(255,255,255,.04);border-radius:9px;margin-bottom:6px'><div><div style='color:#e2e8f0;font-size:13px;font-weight:600'>"+it.name+"</div><div style='color:#475569;font-size:11px'>x"+it.qty+" @ ~"+it.kg_each+"kg = "+tkg.toFixed(2)+"kg</div></div><div style='color:#22d3ee;font-size:15px;font-weight:700'>"+cost.toFixed(0)+" RMB</div></div>";}).join("");res.innerHTML="<div style='background:rgba(34,211,238,.05);border:1px solid rgba(34,211,238,.2);border-radius:12px;padding:14px'>"+rows+"<div style='border-top:1px solid rgba(34,211,238,.2);margin-top:10px;padding-top:10px'><div style='display:flex;justify-content:space-between;align-items:center;margin-bottom:3px'><span style='color:#94a3b8;font-size:13px'>"+tItems+" items - "+tKg.toFixed(2)+"kg total</span><span style='color:#22c55e;font-size:24px;font-weight:700'>"+tRmb.toFixed(0)+" RMB</span></div><div style='text-align:right;color:#475569;font-size:12px'>approx $"+(tRmb/7.25).toFixed(0)+" USD</div></div></div>";}catch(e){res.innerHTML="<span style='color:#ef4444'>Calc failed. Check each item has a line.</span>";}}).catch(function(){res.innerHTML="<span style='color:#ef4444'>Request failed.</span>";});}
function _sfTQC(el){window._qcd={"Sneakers":["Toe box shape correct","Sole thickness correct","Stitching clean","Tongue label correct","Lace color matches","Box barcode correct","Insole correct","No glue marks"],"Clothing":["Tag position correct","Seams even","No loose threads","Material weight right","Zipper brand correct","Print placement correct"],"Bags":["Stitching matches retail","Hardware color correct","Zipper branded","Serial number correct","Lining correct","Leather smell natural"],"Watches":["Crown functions","Crystal clear","Caseback correct","Clasp locks","Dial text correct","Hands aligned"]};var btns=Object.keys(window._qcd).map(function(c){return "<button class='_qb' data-cat='"+c+"' onclick='_qcC(\'"+c+"\')' style='padding:7px 12px;border-radius:8px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#64748b;margin:0 6px 8px 0;font-family:inherit'>"+c+"</button>";}).join("");el.innerHTML=_sH("QC Checklist")+btns+"<div id='_qcl' style='margin-top:4px'></div>";}
function _qcC(cat){document.querySelectorAll("._qb").forEach(function(b){b.style.background="rgba(255,255,255,.05)";b.style.color="#64748b";b.style.borderColor="rgba(255,255,255,.1)";});var ab=document.querySelector("._qb[data-cat='"+cat+"']");if(ab){ab.style.background="rgba(34,211,238,.15)";ab.style.color="#22d3ee";ab.style.borderColor="rgba(34,211,238,.3)";}var items=(window._qcd||{})[cat]||[];document.getElementById("_qcl").innerHTML=items.map(function(t,i){return "<label style='display:flex;gap:9px;align-items:flex-start;padding:8px 0;cursor:pointer;color:#94a3b8;font-size:13px;border-bottom:1px solid rgba(255,255,255,.04)'><input type='checkbox' onchange='_qcU("+items.length+")' style='margin-top:2px;accent-color:#22d3ee;flex-shrink:0'> "+t+"</label>";}).join("")+"<div id='_qcs' style='margin-top:10px;text-align:center;font-size:14px;font-weight:700;padding:8px;border-radius:8px;background:rgba(255,255,255,.03)'>0/"+items.length+"</div>";}
...
function _sfTGloss(el){var terms=[["W2C","Where to Cop - where to buy an item"],["QC","Quality Check - reviewing agent photos"],["GL","Green Light - approve QC, ready to ship"],["1:1","Same quality as retail"],["Haul","Multiple items shipped together"],["PK","Top Putian batch"],["OG","High quality Jordan batch"],["LJR","Dongguan top batch, AJ1s"],["H12","Popular AJ1 batch"],["Chun Yuan","Pure Original - highest Putian tier"],["Gong Si Ji","Company Grade - 95%+ accuracy"],["Zhen Biao","True Label - real tags"],["Pu Tian","Putian city - sneaker capital"],["Middleman","Reseller adding 30-100% markup"],["CNFans","Agent - 0-1.5% fees"],["Sugargoo","Agent - 0% fees, large hauls"],["MOQ","Minimum Order Quantity"],["Yupoo","Chinese photo album for catalogs"]];el.innerHTML=_sH("Rep Glossary")+terms.map(function(t){return "<div style='padding:8px 0;border-bottom:1px solid rgba(255,255,255,.04)'><b style='color:#22d3ee;font-size:12px'>"+t[0]+"</b><div style='color:#64748b;font-size:11px;margin-top:2px'>"+t[1]+"</div></div>";}).join("");}
...
function dySearch(){var brand=(document.getElementById("dy-brand")||{}).value||"",product=(document.getElementById("dy-product")||{}).value||"";if(!brand&&!product){var res=document.getElementById("dy-results");if(res)res.innerHTML="<div style='color:#ef4444;padding:16px;text-align:center'>Enter a brand or product first</div>";return;}var query=dyBuildQ(brand,product,_dyMode);window._dyQ=query;var qd=document.getElementById("dy-query-display"),qt=document.getElementById("dy-query-text");if(qd)qd.style.display="block";if(qt)qt.textContent=query;var res=document.getElementById("dy-results");if(!res)return;res.innerHTML="<div style='text-align:center;padding:24px;color:#94a3b8'>Building...</div>";var enc=encodeURIComponent(query);setTimeout(function(){res.innerHTML="<div style='background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:18px;margin-bottom:12px'><div style='color:#e2e8f0;font-size:14px;font-weight:700;margin-bottom:6px'>&#127279; Douyin Web</div><div style='color:#475569;font-size:12px;margin-bottom:10px'>Search Douyin.com for suppliers. Check video descriptions and comments for WeChat numbers.</div><a href='https://www.douyin.com/search/"+enc+"' target='_blank' style='display:inline-block;padding:10px 20px;background:linear-gradient(135deg,rgba(34,211,238,.2),rgba(99,102,241,.2));border:1px solid rgba(34,211,238,.3);color:#22d3ee;border-radius:9px;font-size:13px;font-weight:700;text-decoration:none'>Open Douyin &#8599;</a></div><div style='background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:18px;margin-bottom:12px'><div style='color:#e2e8f0;font-size:14px;font-weight:700;margin-bottom:8px'>&#128241; Copy for Douyin App</div><div style='display:flex;gap:8px;align-items:center'><div style='flex:1;background:rgba(0,0,0,.3);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:10px;color:#94a3b8;font-size:12px;font-family:monospace;word-break:break-all'>"+query+"</div><button onclick='dyCopy(window._dyQ)' style='padding:10px 14px;background:rgba(34,211,238,.1);border:1px solid rgba(34,211,238,.25);color:#22d3ee;border-radius:8px;font-size:12px;cursor:pointer;white-space:nowrap;font-family:inherit'>Copy</button></div></div><div style='background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:14px;padding:18px'><div style='color:#e2e8f0;font-size:14px;font-weight:700;margin-bottom:10px'>&#128161; What to Look For</div><div style='display:grid;grid-template-columns:1fr 1fr;gap:8px'>"+(["Check descriptions for WeChat","Look for 联系方式 in bios","Comment to ask for WeChat","Watch for factory livestreams","Price lists in videos = suppliers","Accounts with 货源 = source"].map(function(t){return "<div style='background:rgba(34,211,238,.04);border:1px solid rgba(34,211,238,.1);border-radius:8px;padding:8px 10px;color:#94a3b8;font-size:11px'>"+t+"</div>";})).join("")+"</div></div>";},400);}
