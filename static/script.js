...
function _sfTFF(el){
  ...
  h+="<p style='color:#475569;font-size:12px;margin:0 0 14px'>Upload rate sheet. AI reads rates and estimates weights. All in RMB yuan.</p>";
  h+="<div onclick='document.getElementById(\'_ffFile\').click()' style='border:2px dashed rgba(34,211,238,.3);border-radius:12px;padding:14px;text-align:center;margin-bottom:12px;cursor:pointer;background:rgba(34,211,238,.04)'>";
  ...
  h+="<div style='color:#334155;font-size:11px;margin-top:2px'>AI reads rates automatically</div>";
  h+="<input type='file' id='_ffFile' accept='image/*' style='display:none' onchange='_ffLoad(this)'></div>";
  ...
  h+="<div style='color:#22d3ee;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-bottom:7px'>Rates Detected (RMB/kg)</div><div id='_ffRL'></div></div>";
  ...
  h+="<select class='_ffS' style='background:#060c18;border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:8px;border-radius:9px;font-size:12px;outline:none;font-family:inherit'><option value=''>Shipping line...</option></select></div></div>";
  ...
  h+="<button onclick='_ffCalc()' style='flex:2;padding:8px;background:linear-gradient(135deg,rgba(34,211,238,.2),rgba(99,102,241,.2));border:1px solid rgba(34,211,238,.35);color:#22d3ee;border-radius:9px;font-size:13px;cursor:pointer;font-weight:700;font-family:inherit'>Calculate with AI</button></div>";
  h+="<div id='_ffRes'></div>";
  el.innerHTML=h;
}
function _ffLoad(inp){var f=inp.files[0];if(!f)return;var rd=new FileReader();rd.onload=function(e){var img=document.getElementById("_ffImgEl"),w=document.getElementById("_ffImgWrap");if(img)img.src=e.target.result;if(w)w.style.display="block";window._ffB=e.target.result.split(",")[1];window._ffM=f.type||"image/jpeg";_ffScan();};rd.readAsDataURL(f);}
...
function _sfTGloss(el){var t=[["W2C","Where to Cop - where to buy"],["QC","Quality Check - reviewing agent photos"],["GL","Green Light - approve QC, ready to ship"],["1:1","Same quality as retail"],["Haul","Multiple items shipped together"],["PK","Top Putian batch - Yeezy"],["OG","High quality Jordan batch"],["LJR","Dongguan top batch - AJ1s"],["H12","Popular AJ1 batch"],["G Ban","Good for AJ1 low"],["Chun Yuan","Pure Original - highest tier"],["Gong Si Ji","Company Grade - 95%+ accuracy"],["Zhen Biao","True Label - real tags"],["Pu Tian","Putian city - sneaker capital"],["Middleman","Reseller adding 30-100% markup"],["CNFans","Agent - 0-1.5% fees"],["Sugargoo","Agent - 0% fees, large hauls"],["MOQ","Minimum Order Quantity"],["Yupoo","Chinese photo album for catalogs"]];el.innerHTML=_sH("Rep Glossary")+t.map(function(r){return "<div style='padding:7px 0;border-bottom:1px solid rgba(255,255,255,.04)'><b style='color:#22d3ee;font-size:12px'>"+r[0]+"</b><div style='color:#64748b;font-size:11px;margin-top:2px'>"+r[1]+"</div></div>";}).join("");}
