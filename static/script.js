...
function showFreightModal(){
  ...
  if(!m){
    ...
    inn.style.cssText="background:#0f172a;border:1px solid rgba(255,255,255,.12);border-radius:16px;padding:24px;width:90%;max-width:420px;max-height:90vh;overflow-y:auto";
    ...
    inn.appendChild(mk("h3","color:#e2e8f0;margin:0 0 18px;font-size:16px","✈️ Freight Forwarder Search"));
    inn.appendChild(mk("label","color:#94a3b8;font-size:12px;display:block;margin-bottom:4px","Origin"));
    var oi=mk("input","width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px 12px;border-radius:9px;font-size:13px;outline:none;box-sizing:border-box;margin-bottom:12px;font-family:inherit");
    oi.id="_ffOrig";oi.placeholder="putian / guangzhou / shenzhen";inn.appendChild(oi);
    inn.appendChild(mk("label","color:#94a3b8;font-size:12px;display:block;margin-bottom:4px","Destination"));
    var ds=mk("select","width:100%;background:#060c18;border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:9px;font-size:13px;margin-bottom:12px;outline:none;font-family:inherit");
    ds.id="_ffDest";["USA","UK","EU","AU","CA"].forEach(function(v){var o=document.createElement("option");o.value=v;o.textContent=v;ds.appendChild(o);});inn.appendChild(ds);
    inn.appendChild(mk("label","color:#94a3b8;font-size:12px;display:block;margin-bottom:4px","Cargo Type"));
    var cs=mk("select","width:100%;background:#060c18;border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:9px;font-size:13px;margin-bottom:16px;outline:none;font-family:inherit");
    cs.id="_ffCargo";[["replica","Replica / Sensitive"],["general","General goods"]].forEach(function(p){var o=document.createElement("option");o.value=p[0];o.textContent=p[1];cs.appendChild(o);});inn.appendChild(cs);
    var br=mk("div","display:flex;gap:8px;margin-bottom:16px");
    var gb=mk("button","flex:2;padding:10px;background:linear-gradient(135deg,rgba(34,211,238,.2),rgba(99,102,241,.2));border:1px solid rgba(34,211,238,.35);color:#22d3ee;border-radius:9px;font-size:13px;cursor:pointer;font-weight:700;font-family:inherit","Search Forwarders");
    var xb=mk("button","flex:1;padding:10px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#94a3b8;border-radius:9px;font-size:13px;cursor:pointer;font-family:inherit","Cancel");
    xb.onclick=function(){m.style.display="none";};br.appendChild(gb);br.appendChild(xb);inn.appendChild(br);
    var out=mk("div","");out.id="_ffOut";inn.appendChild(out);
    gb.onclick=async function(){
      ...
      try{
        var res=await fetch("/api/freight_search",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},
          body:JSON.stringify({origin:document.getElementById("_ffOrig").value,destination:document.getElementById("_ffDest").value,cargo_type:document.getElementById("_ffCargo").value})});
        var d=await res.json();
        if(!d.results||!d.results.length){out.innerHTML="<div style=\"color:#475569;padding:16px;text-align:center\">No forwarders found.</div>";return;}
        out.innerHTML="";
        d.results.forEach(function(r){
          var card=document.createElement("div");card.style.cssText="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:12px;margin-bottom:8px";
          var tl=document.createElement("div");tl.style.cssText="color:#e2e8f0;font-size:13px;font-weight:600;margin-bottom:4px";tl.textContent=r.title||"Forwarder";card.appendChild(tl);
          var sn=document.createElement("div");sn.style.cssText="color:#475569;font-size:11px;margin-bottom:6px";sn.textContent=(r.snippet||"").slice(0,120);card.appendChild(sn);
          var rw=document.createElement("div");rw.style.cssText="display:flex;justify-content:space-between;align-items:center";
          var sc=document.createElement("span");sc.style.cssText="color:#22d3ee;font-size:12px;font-weight:700";sc.textContent="Score: "+(r.ff_score||0)+"/100";rw.appendChild(sc);
          var wc=(r.wechats&&r.wechats[0])||"";
          if(wc){var ws=document.createElement("span");ws.style.cssText="color:#22d3ee;font-family:monospace;font-size:13px";ws.textContent=wc;rw.appendChild(ws);}
          card.appendChild(rw);
          if(wc){var cb=document.createElement("button");cb.textContent="Copy WeChat";cb.style.cssText="margin-top:6px;padding:4px 10px;background:rgba(34,211,238,.1);border:1px solid rgba(34,211,238,.25);color:#22d3ee;border-radius:6px;font-size:11px;cursor:pointer;font-family:inherit";cb.onclick=(function(w){return function(){navigator.clipboard.writeText(w);};})(wc);card.appendChild(cb);}
...
async function runSmartSearch(){
  ...
  if(typeof showToast!=="undefined")showToast("Smart searching...","info");
  try{
    ...
    if(d.results&&d.results.length){
      ...
      if(rd&&typeof buildCard!=="undefined"){rd.innerHTML="";d.results.forEach(function(item,i){rd.appendChild(buildCard(item,i));});}
      if(typeof showToast!=="undefined")showToast("Query: "+d.enhanced_query,"success");
    }else{if(typeof showToast!=="undefined")showToast(d.error||"No results","error");}
  }catch(e){if(typeof showToast!=="undefined")showToast("Smart search failed","error");}
}

(function initSmartBtn(){
  var ar=document.querySelector(".action-row");
  if(!ar||document.getElementById("_smartBtn")){if(!ar)setTimeout(initSmartBtn,500);return;}
  var b=document.createElement("button");b.id="_smartBtn";b.textContent="🧠 Smart";
  b.style.cssText="margin-left:6px;padding:0 12px;height:36px;background:linear-gradient(135deg,rgba(34,211,238,.15),rgba(99,102,241,.15));border:1px solid rgba(34,211,238,.3);color:#22d3ee;border-radius:9px;font-size:12px;cursor:pointer;font-family:inherit;font-weight:600";
  b.onclick=runSmartSearch;ar.appendChild(b);
})();
// ── Freight & Smart Search UI ─────────────────────────────────────────
function showFreightModal(){
  var m=document.getElementById("_ffModal");
  if(!m){
    ...
    inn.appendChild(mk("label","color:#94a3b8;font-size:12px;display:block;margin-bottom:4px","Destination"));
    var ds=mk("select","width:100%;background:#060c18;border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:9px;font-size:13px;margin-bottom:12px;outline:none;font-family:inherit");
    ds.id="_ffDest";["USA","UK","EU","AU","CA"].forEach(function(v){var op=document.createElement("option");op.value=v;op.textContent=v;ds.appendChild(op);});inn.appendChild(ds);
    inn.appendChild(mk("label","color:#94a3b8;font-size:12px;display:block;margin-bottom:4px","Cargo Type"));
    var cs=mk("select","width:100%;background:#060c18;border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:9px;font-size:13px;margin-bottom:16px;outline:none;font-family:inherit");
    cs.id="_ffCargo";[["replica","Replica / Sensitive"],["general","General goods"]].forEach(function(p){var op=document.createElement("option");op.value=p[0];op.textContent=p[1];cs.appendChild(op);});inn.appendChild(cs);
    var br=mk("div","display:flex;gap:8px;margin-bottom:16px");
    var gb=mk("button","flex:2;padding:10px;background:linear-gradient(135deg,rgba(34,211,238,.2),rgba(99,102,241,.2));border:1px solid rgba(34,211,238,.35);color:#22d3ee;border-radius:9px;font-size:13px;cursor:pointer;font-weight:700;font-family:inherit","Search Forwarders");
    var xb=mk("button","flex:1;padding:10px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#94a3b8;border-radius:9px;font-size:13px;cursor:pointer;font-family:inherit","Cancel");
    xb.onclick=function(){m.style.display="none";};br.appendChild(gb);br.appendChild(xb);inn.appendChild(br);
    var ffOut=mk("div","");ffOut.id="_ffOut";inn.appendChild(ffOut);
    gb.onclick=async function(){
      var ffOut=document.getElementById("_ffOut");
      ffOut.innerHTML="<div style=\"color:#94a3b8;padding:16px;text-align:center\">Searching...</div>";
      try{
        var rr=await fetch("/api/freight_search",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},
          body:JSON.stringify({origin:document.getElementById("_ffOrig").value,destination:document.getElementById("_ffDest").value,cargo_type:document.getElementById("_ffCargo").value})});
        var dd=await rr.json();
        if(!dd.results||!dd.results.length){ffOut.innerHTML="<div style=\"color:#475569;padding:16px;text-align:center\">No forwarders found.</div>";return;}
        ffOut.innerHTML="";
        dd.results.forEach(function(rs){
          var card=document.createElement("div");card.style.cssText="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:10px;padding:12px;margin-bottom:8px";
          var tl=document.createElement("div");tl.style.cssText="color:#e2e8f0;font-size:13px;font-weight:600;margin-bottom:4px";tl.textContent=rs.title||"Forwarder";card.appendChild(tl);
          var sn=document.createElement("div");sn.style.cssText="color:#475569;font-size:11px;margin-bottom:6px";sn.textContent=(rs.snippet||"").slice(0,120);card.appendChild(sn);
          var rw=document.createElement("div");rw.style.cssText="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px";
          var sc=document.createElement("span");sc.style.cssText="color:#22d3ee;font-size:12px;font-weight:700";sc.textContent="Score: "+(rs.ff_score||0)+"/100";rw.appendChild(sc);card.appendChild(rw);
          var wc=(rs.wechats&&rs.wechats[0])||"";
          if(wc){var ws=document.createElement("span");ws.style.cssText="color:#22d3ee;font-family:monospace;font-size:13px";ws.textContent=wc;rw.appendChild(ws);
            var cb=document.createElement("button");cb.textContent="Copy WeChat";cb.style.cssText="margin-top:6px;padding:4px 10px;background:rgba(34,211,238,.1);border:1px solid rgba(34,211,238,.25);color:#22d3ee;border-radius:6px;font-size:11px;cursor:pointer;font-family:inherit";
            cb.onclick=(function(w){return function(){navigator.clipboard.writeText(w);};})(wc);card.appendChild(cb);}
          ffOut.appendChild(card);
        });
      }catch(err){ffOut.innerHTML="<div style=\"color:#ef4444;padding:12px\">Error: "+err.message+"</div>";}
    };
...
async function runSmartSearch(){
  ...
  if(typeof showToast!=="undefined")showToast("Smart searching...","info");
  try{
    var rr2=await fetch("/api/brand_aware_search",{method:"POST",credentials:"include",headers:{"Content-Type":"application/json"},body:JSON.stringify({query:query,brand:brand})});
    var dd2=await rr2.json();
    if(dd2.results&&dd2.results.length){
      var rd=document.getElementById("supplierResults");
      if(rd&&typeof buildCard!=="undefined"){rd.innerHTML="";dd2.results.forEach(function(item,i){rd.appendChild(buildCard(item,i));});}
      if(typeof showToast!=="undefined")showToast("Query: "+dd2.enhanced_query,"success");
    }else{if(typeof showToast!=="undefined")showToast(dd2.error||"No results","error");}
  }catch(err2){if(typeof showToast!=="undefined")showToast("Smart search failed","error");}
}

(function _addSmartBtn(){
  var ar=document.querySelector(".action-row");
  if(!ar){setTimeout(_addSmartBtn,600);return;}
