...
var _sfMO=false;
function _sfMT(){_sfMO=!_sfMO;var p=document.getElementById("_sfNavPanel"),o=document.getElementById("_sfOverlay"),b=document.getElementById("_sfMenuBtn");if(p)p.style.right=_sfMO?"0":"-285px";if(o)o.style.display=_sfMO?"block":"none";if(b)b.style.opacity=_sfMO?"1":".8";}
...
function _sfCT(){var m=document.getElementById("_sfTModal");if(m)m.style.display="none";}
document.addEventListener("keydown",function(e){if(e.key==="Escape"){_sfCT();if(_sfMO){_sfMO=false;var p=document.getElementById("_sfNavPanel"),o=document.getElementById("_sfOverlay");if(p)p.style.right="-285px";if(o)o.style.display="none";}}});
function _sH(t){return "<h3 style='color:#e2e8f0;margin:0 0 16px;font-size:16px;font-weight:700'>"+t+"</h3>";}
function _sI(id,ph){return "<input id='"+id+"' type='number' placeholder='"+ph+"' class='_In'>";}
function _sS(id,opts,oc){return "<select id='"+id+"'"+(oc?" onchange='"+oc+"()'":" ")+" class='_Se'>"+opts+"</select>";}
function _sB(l,fn){return "<button onclick='"+fn+"()' class='_Bt'>"+l+"</button>";}
function _sO(id){return "<div id='"+id+"' class='_Ou'></div>";}
function _sfTCny(el){el.innerHTML=_sH("CNY Converter")+_sI("_cn","Amount in CNY")+"<div style='display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:10px'><button onclick='_cCnv(\'USD\')' class='_Bt' style='margin:0'>USD</button><button onclick='_cCnv(\'EUR\')' class='_Bt' style='margin:0'>EUR</button><button onclick='_cCnv(\'GBP\')' class='_Bt' style='margin:0'>GBP</button></div>"+_sO("_cr")+"<p style='color:#1e293b;font-size:10px;margin-top:6px;text-align:center'>~7.25 CNY/USD  ~7.8/EUR  ~9.1/GBP</p>";}
...
function _sfTLink(el){el.innerHTML=_sH("Agent Link Converter")+"<textarea id='_lnk' placeholder='Paste Taobao / Weidian / 1688 URL...' style='width:100%;height:72px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#e2e8f0;padding:9px;border-radius:10px;font-size:12px;resize:none;outline:none;box-sizing:border-box;margin-bottom:9px;font-family:inherit'></textarea>"+_sB("Generate All Agent Links","_sLnkG")+"<div id='_lnkr'></div>";}
function _sLnkG(){var url=((document.getElementById("_lnk")||{}).value||"").trim(),r=document.getElementById("_lnkr");if(!url||!r){if(r)r.innerHTML="<span style='color:#ef4444'>Paste a URL first</span>";return;}var e=encodeURIComponent(url);var ag=[["CNFans","https://cnfans.com/?num="+e],["Kakobuy","https://www.kakobuy.com/item/details?url="+e],["Sugargoo","https://www.sugargoo.com/#/home/productDetail?productLink="+e],["ACBuy","https://acbuy.com/product?url="+e],["Pandabuy","https://www.pandabuy.com/product?url="+e],["Superbuy","https://www.superbuy.com/en/page/buy/?url="+e],["Hagobuy","https://www.hagobuy.com/item/details?url="+e],["Mulebuy","https://mulebuy.com/product/?url="+e]];r.innerHTML=ag.map(function(a){return "<a href='"+a[1]+"' target='_blank' class='_Lk'>"+a[0]+" &#8594;</a>";}).join("");}
function _sfTFF(el){
  el.innerHTML=_sH("FF Rate Calculator");
  el.innerHTML+="<label style='display:block;background:rgba(34,211,238,.1);border:1px solid rgba(34,211,238,.25);color:#22d3ee;padding:10px;border-radius:10px;text-align:center;cursor:pointer;margin-bottom:12px;font-size:13px;font-weight:600'>&#128247; Upload Rate Sheet Photo<input type='file' accept='image/*' id='_ffImg' style='display:none' onchange='_ffPrev(this)'></label>";
  el.innerHTML+="<div id='_ffPrevDiv' style='display:none;margin-bottom:12px'><img id='_ffPrevImg' style='width:100%;border-radius:10px;border:1px solid rgba(255,255,255,.1);max-height:200px;object-fit:contain;background:#000'></div>";
  el.innerHTML+="<div id='_ffRows'><div class='_ffRow' style='display:grid;grid-template-columns:1fr 60px 70px 28px;gap:6px;margin-bottom:8px'><input class='_ffi _ffName' placeholder='Item (e.g. Jordan 4)'><input class='_ffi _ffQty' type='number' placeholder='Qty' min='1'><input class='_ffi _ffKg' type='number' placeholder='kg' step='.1' min='.1'><button onclick='_ffRem(this)' style='background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#f87171;border-radius:8px;cursor:pointer;font-size:14px'>x</button></div></div>";
  el.innerHTML+="<div style='display:flex;gap:8px;margin-bottom:10px'><button onclick='_ffAdd()' style='flex:1;padding:8px;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#94a3b8;border-radius:8px;font-size:12px;cursor:pointer'>+ Add Item</button>"+_sS("_ffRate","<option value=''>Rate ($/kg)</option><option value='3'>$3/kg economy</option><option value='5'>$5/kg standard</option><option value='8'>$8/kg express</option><option value='12'>$12/kg DHL</option><option value='18'>$18/kg FedEx</option><option value='custom'>Custom</option>","_ffRateSel")+"<input id='_ffCust' type='number' placeholder='$/kg' style='display:none;width:70px' class='_In'></div>";
  el.innerHTML+=_sB("Calculate Total","_ffCalc")+"<div id='_ffRes'></div>";
}
function _ffPrev(inp){var f=inp.files[0];if(!f)return;var rd=new FileReader();rd.onload=function(e){var img=document.getElementById("_ffPrevImg"),div=document.getElementById("_ffPrevDiv");if(img)img.src=e.target.result;if(div)div.style.display="block";};rd.readAsDataURL(f);}
function _ffAdd(){var r=document.getElementById("_ffRows");if(!r)return;var d=document.createElement("div");d.className="_ffRow";d.style.cssText="display:grid;grid-template-columns:1fr 60px 70px 28px;gap:6px;margin-bottom:8px";d.innerHTML="<input class='_ffi _ffName' placeholder='Item name'><input class='_ffi _ffQty' type='number' placeholder='Qty' min='1'><input class='_ffi _ffKg' type='number' placeholder='kg' step='.1' min='.1'><button onclick='_ffRem(this)' style='background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.2);color:#f87171;border-radius:8px;cursor:pointer;font-size:14px'>x</button>";r.appendChild(d);}
function _ffRem(btn){var rows=document.querySelectorAll("._ffRow");if(rows.length<=1)return;btn.closest("._ffRow").remove();}
function _ffRateSel(){var v=(document.getElementById("_ffRate")||{}).value,c=document.getElementById("_ffCust");if(c)c.style.display=v==="custom"?"inline-block":"none";}
function _ffCalc(){var rv=(document.getElementById("_ffRate")||{}).value,rate=rv==="custom"?parseFloat((document.getElementById("_ffCust")||{}).value||0):parseFloat(rv||0),res=document.getElementById("_ffRes");if(!rate||!res){if(res)res.innerHTML="<span style='color:#ef4444'>Select a rate per kg first</span>";return;}var rows=document.querySelectorAll("._ffRow"),items=[],tkgs=0,titems=0;rows.forEach(function(row){var name=(row.querySelector("._ffName")||{}).value||"Item",qty=parseInt((row.querySelector("._ffQty")||{}).value||1),kg=parseFloat((row.querySelector("._ffKg")||{}).value||0);if(kg>0){var tkg=kg*qty,cost=tkg*rate;tkgs+=tkg;titems+=qty;items.push({name:name,qty:qty,kg:kg,tkg:tkg,cost:cost});}});if(!items.length){res.innerHTML="<span style='color:#ef4444'>Enter item weights first</span>";return;}var total=tkgs*rate;var rows2=items.map(function(it){return "<div style='display:flex;justify-content:space-between;align-items:center;padding:8px 10px;background:rgba(255,255,255,.04);border-radius:8px;margin-bottom:5px'><div><div style='color:#e2e8f0;font-size:13px;font-weight:600'>"+it.name+"</div><div style='color:#475569;font-size:11px'>x"+it.qty+" @ "+it.kg+"kg = "+it.tkg.toFixed(2)+"kg</div></div><div style='color:#22d3ee;font-size:14px;font-weight:700'>$"+it.cost.toFixed(2)+"</div></div>";}).join("");res.innerHTML="<div style='background:rgba(34,211,238,.05);border:1px solid rgba(34,211,238,.2);border-radius:12px;padding:12px'>"+rows2+"<div style='border-top:1px solid rgba(34,211,238,.2);margin-top:8px;padding-top:8px;display:flex;justify-content:space-between'><span style='color:#94a3b8;font-size:13px'>"+titems+" items  "+tkgs.toFixed(2)+"kg</span><span style='color:#22c55e;font-size:20px;font-weight:700'>$"+total.toFixed(2)+"</span></div></div>";}
function _sfTQC(el){window._qcd={"Sneakers":["Toe box shape correct","Sole thickness correct","Stitching clean","Tongue label correct","Lace color matches","Box barcode correct","Insole correct","No glue marks"],"Clothing":["Tag position correct","Seams even","No loose threads","Material weight right","Zipper brand correct","Print placement correct"],"Bags":["Stitching matches retail","Hardware color correct","Zipper branded","Serial number correct","Lining color correct","Leather smell natural"],"Watches":["Crown functions","Crystal clear","Caseback correct","Clasp locks","Dial text correct","Hands aligned"]};var btns=Object.keys(window._qcd).map(function(c){return "<button class='_qb' data-cat='"+c+"' onclick='_qcC(\'"+c+"\')' style='padding:7px 12px;border-radius:8px;font-size:12px;cursor:pointer;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);color:#64748b;margin:0 6px 8px 0;font-family:inherit'>"+c+"</button>";}).join("");el.innerHTML=_sH("QC Checklist")+btns+"<div id='_qcl' style='margin-top:4px'></div>";}
...
function _sfTGloss(el){var terms=[["W2C","Where to Cop - where to buy an item"],["QC","Quality Check - reviewing agent photos before shipping"],["GL","Green Light - approve QC, ready to ship"],["1:1","Same quality as retail"],["Haul","Multiple items shipped together"],["PK","Top Putian batch, started with Yeezy 350"],["OG","High quality Jordan batch"],["LJR","Dongguan top batch, excellent AJ1s"],["H12","Popular AJ1 batch, good value"],["G Ban","Good for AJ1 low"],["Chun Yuan","Pure Original - highest Putian tier"],["Gong Si Ji","Company Grade - 95%+ accuracy"],["Zhen Biao","True Label - real tags"],["Pu Tian","Putian city - China sneaker capital"],["Middleman","Reseller adding 30-100% markup"],["CNFans","Agent - 0-1.5% fees, best overall"],["Sugargoo","Agent - 0% fees, large hauls"],["MOQ","Minimum Order Quantity"],["Yupoo","Chinese photo album for catalogs"],["Consolidation","Combining parcels to save shipping"]];el.innerHTML=_sH("Rep Glossary A-Z")+terms.map(function(t){return "<div style='padding:8px 0;border-bottom:1px solid rgba(255,255,255,.04)'><b style='color:#22d3ee;font-size:12px'>"+t[0]+"</b><div style='color:#64748b;font-size:11px;margin-top:2px'>"+t[1]+"</div></div>";}).join("");}
