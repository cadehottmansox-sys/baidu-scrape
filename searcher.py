...
# ========== BRAND ENRICHMENT & FREIGHT ENHANCEMENTS ==========
...
PRODUCT_BRAND_CACHE = {
    ...
    "jordan": {"brand": "Jordan", "chinese_name": "乔丹", "category": "sneaker"},
    "yeezy": {"brand": "Adidas", "chinese_name": "椰子", "category": "sneaker"},
    "dunk": {"brand": "Nike", "chinese_name": "耐克", "category": "sneaker"},
    "tech fleece": {"brand": "Nike", "chinese_name": "耐克", "category": "clothing"},
    "hellstar": {"brand": "Hellstar", "chinese_name": "地狱星", "category": "clothing"},
    "sp5der": {"brand": "Sp5der", "chinese_name": "蜘蛛", "category": "clothing"},
    "lego": {"brand": "LEGO", "chinese_name": "乐高", "category": "lego"},
    "lepin": {"brand": "Lepin", "chinese_name": "乐拼", "category": "lego"},
}

def enrich_product_with_brand(product_name):
    ...
    for key, info in PRODUCT_BRAND_CACHE.items():
        if key in product_lower:
            ...
            return result
    return {"brand": "", "chinese_name": "", "category": "general", "source": "none"}
...
def build_brand_aware_query(query, brand=""):
    ...
    if enrichment.get("brand"):
        ...
        if category == "toy":
            return f"{query} {chinese_brand} 玩具厂 硅胶 工厂 微信 一手货源"
        elif category == "sneaker":
            return f"{query} {chinese_brand} 鞋厂 莆田 运动鞋 过验 微信"
        elif category == "clothing":
            return f"{query} {chinese_brand} 服装厂 纯原 过验 微信 一手货源"
        elif category == "lego":
            return f"{query} {chinese_brand} 积木厂 兼容 工厂 批发 微信"
        return f"{query} {chinese_brand} 厂家 微信"
    if brand:
        return f"{query} {brand} 厂家 微信 一手货源"
...
def build_freight_query(origin="", destination="USA", cargo_type="replica"):
    ...
    if route_key in FREIGHT_ROUTES:
        parts.extend(FREIGHT_ROUTES[route_key])
    if cargo_type in ["replica", "sensitive"]:
        parts.extend(["敏感货", "仿牌", "双清包税", "DDP"])
    hub_key = (origin or "").lower()
    if hub_key in FREIGHT_HUBS:
        parts.extend(FREIGHT_HUBS[hub_key])
    parts.append("微信")
    seen = set()
    unique = []
    for p in parts:
...
def score_freight_forwarder(text):
    score = 0
    text_lower = text.lower()
    if any(k in text_lower for k in ["敏感货", "仿牌"]):
        score += 25
    if any(k in text_lower for k in ["双清包税", "包清关", "ddp"]):
        score += 20
    if any(k in text_lower for k in ["美国专线", "中美专线"]):
        score += 15
    if any(k in text_lower for k in ["莆田", "广州", "义乌", "深圳"]):
        score += 10
    if any(c in text_lower for c in ["fedex", "dhl", "ups"]):
        score += 5
    if any(k in text_lower for k in ["不接仿牌", "只接普货", "只做普货"]):
        score += 30
