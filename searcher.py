...
# ========== BRAND ENRICHMENT & FREIGHT HELPERS ==========
...
def enrich_product_with_brand(product_name):
    ...
    for key, info in PRODUCT_BRAND_CACHE.items():
        if key in pl:
            r = info.copy(); r['source'] = 'cache'; return r
    return {"brand":"","chinese_name":"","category":"general","source":"none"}

def build_brand_aware_query(query, brand=""):
    e = enrich_product_with_brand(query)
    cn = e.get("chinese_name","") or brand
    cat = e.get("category","general")
    if cat == "toy":    return f"{query} {cn} 玩具厂 硅胶 工厂 微信 一手货源"
    if cat == "sneaker": return f"{query} {cn} 鞋厂 莆田 运动鞋 过验 微信"
    if cat == "clothing": return f"{query} {cn} 服装厂 纯原 过验 微信 一手货源"
    if cat == "lego":   return f"{query} {cn} 积木厂 兼容 工厂 批发 微信"
    if cn:              return f"{query} {cn} 厂家 微信 一手货源"
    return f"{query} 厂家 微信 一手货源"

FREIGHT_ROUTES = {
    "USA":["美国专线","中美专线"],"UK":["英国专线","中英专线"],
    "EU":["欧洲专线"],"AU":["澳洲专线"],"CA":["加拿大专线"],
}
FREIGHT_HUBS = {
    "putian":["莆田货运","莆田货代"],"guangzhou":["广州货代"],
    "yiwu":["义乌货代"],"shenzhen":["深圳货代"],
}

def build_freight_query(origin="", destination="USA", cargo_type="replica"):
    parts = ["货代","货运代理"]
    parts.extend(FREIGHT_ROUTES.get(destination.upper(),[]))
    if cargo_type in ["replica","sensitive"]:
        parts.extend(["敏感货","仿牌","双清包税","DDP"])
    parts.extend(FREIGHT_HUBS.get((origin or "").lower(),[]))
    parts.append("微信")
    seen=set(); out=[]
    for p in parts:
